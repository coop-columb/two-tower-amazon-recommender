...
def load_category_data(data_dir: Path) -> Dict[str, pd.DataFrame]:
    ...
    parquet_files = list(data_dir.glob("*_reviews.parquet")) + list(data_dir.glob("*_5core.parquet"))
    ...
    for file_path in parquet_files:
        ...
        if 'rating' in df.columns:
            # Full reviews data
            required_cols = ['user_id', 'parent_asin', 'rating', 'timestamp', 'category']
...
def main() -> None:
    ...
    logger.info(f"  Range: {stats['user_stats']['min_interactions']}-{stats['user_stats']['max_interactions']}")
    
    logger.info(f"\nItem statistics:")
    logger.info(f"  Avg interactions: {stats['item_stats']['avg_interactions']:.1f}")
    logger.info(f"  Range: {stats['item_stats']['min_interactions']}-{stats['item_stats']['max_interactions']}")
