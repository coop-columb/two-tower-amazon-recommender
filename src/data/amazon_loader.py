        ...
        if 'rating' in data.columns:
            ...
            if 'rating' in data.columns:
                ...
                if len(invalid_ratings) > 0:
                    issues.append(f"Found {len(invalid_ratings)} invalid ratings outside 1-5 range")
            
            # Text quality validation
            if 'text' in data.columns:
                empty_text = data[data['text'].isnull() | (data['text'].str.strip() == '')]
                if len(empty_text) > len(data) * 0.1:  # More than 10% empty
                    issues.append(f"High percentage of empty reviews: {len(empty_text)/len(data)*100:.1f}%")
...
class AmazonReviewsLoader(DataLoader):
    ...
    def get_available_categories(self) -> List[str]:
        ...
        try:
            ...
            available_configs = dataset_info.config_names or []
    ...
    def get_dataset_statistics(self, categories: Optional[List[str]] = None) -> Dict[str, Dict]:
        """
        ...
        for category in categories:
            try:
                ...
                category_stats = {
                    ...
                    "avg_text_length": df["text"].str.len().mean() if "text" in df.columns else None,
                }
