...
class TextProcessor:
    ...
    def clean_text(self, text: Union[str, None, Any]) -> str:
        """
...
class FeatureEngineer:
    ...
    def create_item_features(
        ...
        item_stats = df.groupby('parent_asin').agg({
            'rating': ['count', 'mean', 'std', 'min', 'max'],
            'text_length': ['mean', 'std'],
            'word_count': ['mean', 'std']
        }).round(3)
...
class AmazonReviewsPreprocessor(DataProcessor):
    ...
    def __init__(self, config: DatasetConfig, logger: Optional[logging.Logger] = None):
    ...
    def _log_dataset_statistics(self, df: pd.DataFrame) -> None:
        ...
        stats = {
            ...
            "sparsity": 1 - (len(df) / (
                df['user_id'].nunique() * df['parent_asin'].nunique()
            )),
